...
import { dbConfig } from './env'; // Assurez-vous que env.ts est bien typé
...
const connect = async () => {
    try {
        ...
        console.log(`Connection has been established successfully to db: ${dbConfig.host} port: ${dbConfig.port}`);
    }
    catch (error) {
        console.error(`Unable to connect to the database ${dbConfig.host} port ${dbConfig.port}: `, error);
        ...
        try {
            ...
            console.log(`Connection has been established successfully to db: ${dbConfig.host} port: 5432`);
        }
        catch (error) {
            console.error(`Unable to connect to the database ${dbConfig.host} port 5432: `, error);
